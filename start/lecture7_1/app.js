...
class App{
        ...
        function onSelect() {
            if (self.reticle.visible){
                ...
                if (self.measurements.length == 2) {
                    ...
                    const distance = Math.round( self.getDistance(
                        self.measurements
                     ) * 100 );
    ...
    getHitTestResults( frame ){
        ...
        if ( hitTestResults.length ) {
            ...
            if( this.currentLine ) this.updateLine (this.reticle.matrix,
                this.currentLine );
...
label.div.style.transform = `translate(-50%, -5-%) translate(${pos.x}px, ${pos.y}px)`;
})
